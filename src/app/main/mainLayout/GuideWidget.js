...
const SubBoard = ({ number, title, content }) => {
    return (
        <div className="flex flex-col items-center p-5 rounded-2xl bg-[#332A6A] md:p-7">
            <div className="bg-primary w-6 rounded-full">{number}</div>
            <div>
...
export const GuideWidget = () => {
    return (
        <div className="my-28">
            <h3 class="text-center text-3xl font-extrabold italic uppercase md:text-6xl"> HOW TO BUY VAULTFI GUIDE</h3>
            <div className="grid grid-cols-3 gap-20 my-28 px-10">
                ...
                />
                <SubBoard
                    content={"Enter the amount you want to buy"}
                    number={4}
                />
                <SubBoard
                    content={" Confirm the transaction on your wallet, make sure you have enough ETH to cover gas fee"}
                    number={5}
                />
                <SubBoard
            ...
            <p class="text-sm text-center font-medium text-white/50">
                Note: For USDT & USDC, you need to first approve and then send, which means there are 2 transactions that you need to approve.
            </p>
        </div>
    );
}