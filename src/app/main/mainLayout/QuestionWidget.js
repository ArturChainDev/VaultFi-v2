...
//   { question: 'Where will I be able to exchange and trade my VaultFi tokens?' },

const QuestionBoard = ({ children, question }) => {
  const [isDropDown, setDropDown] = useState(false);
  return (
    <div className="my-3">
      <h3
        class="mb-4 pb-2 text-uppercase text-3xl cursor-pointer"
        onClick={() => setDropDown(!isDropDown)}
      >
        {question}
      </h3>
      <img src="" />
      {isDropDown && <div>{children}</div>}
    </div>
  );
};
    ...
    <div class="container">
      <div class="md:grid md:grid-cols-2">
        ...
        <div class="">
          ...
          <QuestionBoard question={'What are the core goals of VaultFi?'}>
            <p className="text-left text-xl p-1">
          ...
          <QuestionBoard question={'How can the APY be sustainable?'}>
            <p className="text-left text-xl p-1">
              VaultFi aims to maintain a sustainable APY through several
              innovative strategies:
            </p>
            <p className="text-left text-xl p-1">
            <span className="text-2xl">Diversified Yield Sources:</span> VaultFi introduces various sources of
              yield beyond the traditional sell tax. This includes integrating
              projects from its Web3 incubator, CoinRock Ventures, into its
              portfolio, which helps diversify and enhance the returns.
            </p>
            <p className="text-left text-xl p-1">
            <span className="text-2xl">Tax Revenue System:</span> In the initial phase, VaultFi employs a tax
              revenue system where a percentage of each sell transaction is
              funneled into the treasury. This constant inflow helps support the
              treasury's capacity to provide yield, regardless of market
              conditions.
            </p>
            <p className="text-left text-xl p-1">
            <span className="text-2xl">Phased Growth Strategy:</span> The roadmap of VaultFi is structured into
              phases that gradually build up the ecosystem's robustness. Each
              phase introduces more assets and strategies for yield generation,
              reducing reliance on any single source and spreading risk.
            </p>
            <p className="text-left text-xl p-1">
            <span className="text-2xl">Multi-Vault Diversification:</span> In later phases, VaultFi plans to
              further diversify its APY sources by incorporating multiple Vaults
              that include a variety of digital assets and projects. This not
              only spreads risk but also creates more opportunities for earning
              returns.
            </p>
            <p className="text-left text-xl p-1">
            <span className="text-2xl">Active Treasury Management:</span> The treasury's active management
              ensures that funds are allocated efficiently to sustain APY. This
              includes investing in profitable ventures and managing assets in a
              way that balances growth with yield provision.
            </p>
            <p className="text-left text-xl p-1">
